...
import priceLabel from '~/assets/images/ellipse_label.png';
...
const isReversed = computed(() => currentIndex.value % 2 === 1);
...
<template>
  <div class="w-full h-[60vh] relative flex justify-center items-center overflow-hidden"
    :style="{ backgroundImage: `url(${bg_SaleBar})` }">
    <div class="offer-slide w-full max-w-4xl flex flex-row items-center justify-between z-10 px-6"
      :class="{ 'flex-row-reverse': isReversed }" @mouseenter="stopAutoplay" @mouseleave="startAutoplay">
      ...
      <div class="w-1/2 flex justify-center relative">
        <img :src="currentOffer?.product_img" class="h-[250px] object-contain z-10" />
        <div :class="[
          'absolute -bottom-8 w-[120px] h-[70px] bg-no-repeat bg-cover bg-center flex flex-col justify-center items-center z-20',
          isReversed ? '-left-8' : '-right-8'
        ]"
        :style="{backgroundImage:`url(${priceLabel})`}"
        >
          <span class="text-white text-[16px] font-light font-[Roboto]">{{ currentOffer?.priceTitle }}</span>
          <span class="text-white text-[20px] font-medium font-[Roboto]">{{ currentOffer?.price.toLocaleString() }}</span>
        </div>
      </div>
    </div>

    <div class="absolute bottom-6 flex flex-row gap-x-3 z-20">
      <div v-for="(offer, i) in offersData" :key="offer.id + '-' + i"
        class="w-3 h-3 rounded-full cursor-pointer transition-all duration-300"
        :class="i === currentIndex ? 'bg-[#FF8400]' : 'border border-[#FF8400] bg-transparent'" @click="goTo(i)" />
    </div>
